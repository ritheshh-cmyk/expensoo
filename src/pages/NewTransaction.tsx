  ...
  const handleSubmit = async () => {
    ...
    try {
      const transformedData = {
        ...
        change_returned: Math.max(0, data.amountGiven - data.repairCost),
        remarks: data.remarks || '',
        parts_cost: data.parts.reduce((sum, part) => sum + part.cost, 0),
        free_glass_installation: data.freeGlass,
        repair_service_type: data.repairServiceType,
