...
export default function Transactions() {
  ...
  const loadTransactions = async () => {
    try {
      ...
      const transformedData: Transaction[] = transactions.map((t: any) => ({
        id: t.id || `TXN${Date.now()}`,
        date: new Date(t.date || t.created_at || Date.now()),
        customer: t.customer_name || t.customerName || t.customer || "Unknown",
        phone: t.mobile_number || t.mobileNumber || t.phone || "",
        device: t.device_model || t.deviceModel || t.device || "Unknown Device",
        repairType: t.repair_type || t.repairType || t.service || "General Repair",
        cost: parseFloat(t.repair_cost || t.repairCost || t.cost || t.total || 0),
        paymentMethod: (t.payment_method || t.paymentMethod || "cash").toLowerCase(),
        status: (t.status || "pending").toLowerCase(),
        freeGlass: t.free_glass_installation || t.freeGlass || false,
      }));
      ...
      toast({
        ...
        description: "Unable to load transactions from server. Showing sample data.",
        variant: "default",
      });
    } finally {
      setLoading(false);
