...
export function MobileBottomNav() {
  ...
  let navItems = visibleNavigation;
  if (navItems.length > 5) {
    const adminItem = visibleNavigation.find(n => n.name === 'admin');
    const settingsItem = visibleNavigation.find(n => n.name === 'settings');
    const baseItems = visibleNavigation.filter(n => n.name !== 'admin' && n.name !== 'settings');
    
    if (adminItem) {
      navItems = [...baseItems.slice(0, 3), settingsItem, adminItem].filter(Boolean) as typeof visibleNavigation;
    } else {
      navItems = [...baseItems.slice(0, 4), settingsItem].filter(Boolean) as typeof visibleNavigation;
    }
  }
